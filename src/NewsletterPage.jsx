...
const BRAND = {
  authorName: import.meta.env.VITE_AUTHOR_NAME ?? "",
  authorTitle: import.meta.env.VITE_AUTHOR_TITLE ?? "",
  portfolioUrl: import.meta.env.VITE_PORTFOLIO_URL ?? "",
  linkedinUrl: import.meta.env.VITE_LINKEDIN_URL ?? "",
  githubUrl: import.meta.env.VITE_GITHUB_URL ?? "",
  footerTagline: import.meta.env.VITE_FOOTER_TAGLINE ?? "",
  newsletterName: import.meta.env.VITE_NEWSLETTER_NAME ?? "",
};
    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)] font-body antialiased page-gradient">
      ...
      <header className="relative pt-10 pb-6">
        ...
        <div className="max-w-xl mx-auto px-5 flex flex-col items-center text-center">
          <a href={BRAND.portfolioUrl} target="_blank" rel="noopener noreferrer" className="block mb-4">
            <img
              src={`${BRAND.portfolioUrl.replace(/\/$/, "")}/assets/profile-header.jpeg`}
          ...
          <p className="text-sm font-medium text-[var(--accent)] tracking-wide uppercase mb-1">{BRAND.authorTitle}</p>
          <h1 className="font-heading text-3xl md:text-4xl font-semibold tracking-tight text-[var(--text)]">
            {BRAND.authorName}
          </h1>
          <p className="mt-1 font-heading text-xl text-[var(--text-muted)]">{BRAND.newsletterName}</p>
          ...
          <div className="mt-4 flex gap-3">
            <a href={BRAND.linkedinUrl} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full border border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--accent)] hover:border-[var(--accent)]/50 transition-colors" aria-label="LinkedIn">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
            </a>
            <a href={BRAND.githubUrl} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full border border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--accent)] hover:border-[var(--accent)]/50 transition-colors" aria-label="GitHub">
            ...
            <a href={BRAND.portfolioUrl} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full border border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--accent)] hover:border-[var(--accent)]/50 transition-colors" aria-label="Portfolio">
      ...
      <footer className="py-10 text-center border-t border-[var(--border)]">
        <p className="text-[var(--text-muted)] text-sm mb-2">{BRAND.authorName} · {BRAND.authorTitle.split(" · ")[0]}</p>
        <p className="text-[var(--text-muted)] text-xs mb-3">{BRAND.footerTagline}</p>
        <a href={BRAND.portfolioUrl} target="_blank" rel="noopener noreferrer" className="text-[var(--accent)] hover:underline text-sm">View full portfolio →</a>
      </footer>
    </div>
  );
}
  ...
  if (submitted) {
    return (
      <div className="animate-success rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-10 text-center shadow-sm">
        ...
        <a href={BRAND.portfolioUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-4 text-sm text-[var(--accent)] hover:underline">Explore my portfolio →</a>
      </div>
    );
  }
